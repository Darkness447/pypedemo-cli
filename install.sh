...
if [[ ":$PATH:" != *":$HOME/.local/bin:"* ]]; then
  ...
  if [ -n "$RC_FILE" ]; then
    ...
    echo "Please run 'source $RC_FILE' after installation or start a new terminal."
  fi
fi
...
# Create the pype script
cat > "$INSTALL_DIR/pype" << 'EOF'
#!/bin/bash
...
chmod +x "$INSTALL_DIR/pype"

# Check for jq dependency
if ! command -v jq &> /dev/null; then
  echo "Warning: jq is not installed and is required for this demo."
...
echo "Pype CLI Demo has been installed successfully!"
echo "You can now use the 'pype' command to interact with the demo CLI."
echo ""
echo "Example usage:"
echo "  pype login"
echo "  pype projects list"